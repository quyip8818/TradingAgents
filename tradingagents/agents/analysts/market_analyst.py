...
def create_market_analyst(llm):
    ...
    def market_analyst_node(state):
        ...
        system_message = (
            """你是一位负责分析金融市场的交易助手。你的角色是从以下列表中选择最适合给定市场条件或交易策略的**最相关指标**。目标是选择最多**8个指标**，这些指标提供互补的见解而不冗余。类别和每个类别的指标如下：
...
- close_10_ema: 10日指数移动平均线：响应迅速的短期平均线。用法：捕捉动量的快速变化和潜在入场点。提示：在震荡市场中容易产生噪音；与更长期的平均线一起使用以过滤虚假信号。
...
- 选择提供多样化和互补信息的指标。避免冗余（例如，不要同时选择rsi和stochrsi）。还要简要解释为什么它们适合给定的市场背景。当你进行工具调用时，请使用上面提供的指标的精确名称，因为它们是定义的参数，否则你的调用将失败。请确保首先调用get_stock_data以检索生成指标所需的CSV。然后使用get_indicators和特定的指标名称。编写一份非常详细和细致的报告，说明你观察到的趋势。不要简单地说明趋势是混合的，提供详细和细粒度的分析和见解，可能有助于交易员做出决策。"""
